...
1. **Real AdMob integration** — the power-up ad flow is fully built but stubbed; pure infrastructure work (AdMob account + SPM package + swap `MockAdRewardProvider`), not a design decision, and the one item blocking the power-up system from generating any actual revenue. **Blocked on:** an AdMob account + the SDK added via Xcode's SPM UI (can't be done headlessly). *This is now the top item — it's a hard dependency for both real revenue and the power-up first-earn prompt below.*
